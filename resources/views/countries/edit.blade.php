...
@section('description', __('Edit an existing country'))
...
<div class="container-fluid pb-30 pt-30">
    <div class="card mb-80">
        ...
        <div class="card-body">
            <div class="row">
                <form action="{{ route('countries.update', $country->id) }}" method="POST" enctype="multipart/form-data" class="col-11 m-auto">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="tab-content" id="tabcontent2">
                        ...
                        if(isset($country))
                        {
                        $currentLanguageCountry = $country->countryLanguage()->where('language_id',
                        $language->id)->first();
                        }
                        @endphp
                        <div class="tab-pane fade {{$language->is_default ? 'show active':''}} " id="tabs-icons-text-{{$language->id}}" role="tabpanel" aria-labelledby="tabs-icons-text-{{$language->id}}-tab">
                            <div class="form-group">
                                ...
                                <input type="text" name="countries[{{$language->id}}][name]" class="form-control @error('name') is-invalid @enderror" value="{{old('name', $currentLanguageCountry->name ?? '')}}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="short_name" class="form-control @error('short_name') is-invalid @enderror" value="{{ old('short_name', $country->short_name) }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="language_id" class="form-control">
                            ...
                            @foreach ($languages as $language)
                            <option value="{{ $language->id }}" {{ old('language_id', $country->language_id) == $language->id ? 'selected' : ''
                    ...
                    <div class="form-group">
                        <label>{{ __('Flag') }}</label>
                        <br>
                        <x-image src="{{ $country->flag }}" alt="{{ $country->name }} {{ __('Flag') }}" />
