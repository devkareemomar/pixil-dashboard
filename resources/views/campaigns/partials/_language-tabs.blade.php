                ...
                <div class="tab-content" id="tabcontent2">
                    @foreach(\App\Models\Language::get() as $language)
                        @php
                            if(isset($result))
                            {
                            $currentLanguageCampaign = $result->languageCampaigns()->where('language_id',
                            $language->id)->first();
                            }
                        @endphp
                            ...
                            <div class="row">
                                <div class="form-group col-6">
                                    ...
                                    <input {{ $language->is_default ? 'required':'' }}
                                           type="text" name="campaigns[{{$language->id}}][title]" class="form-control"
                                           value="{{old('title', $currentLanguageCampaign->title ?? '')}}">
                                    @error('title')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="form-group col-6">
                                    <label for="slogan">{{__('Slogan')}}
                                        @if($language->is_default)
                                            <span class="text-danger">*</span>
                                        @endif
                                    </label>
                                    <input {{ $language->is_default ? 'required':'' }} type="text"
                                           name="campaigns[{{$language->id}}][slogan]"
                                           class="form-control"
                                           id="slogan" value="{{
								old('slogan',$currentLanguageCampaign->slogan ?? '') }}">
                                    @error('slogan')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea {{
									$language->is_default ? 'required':'' }} type="text"
                                          name="campaigns[{{$language->id}}][description]" class="form-control"
                                          id="description-{{$language->id}}">{{ old('description',$currentLanguageCampaign->description ?? '') }}</textarea>
                                @error('description')
                                <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
