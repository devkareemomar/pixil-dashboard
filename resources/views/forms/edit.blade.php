...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card mb-80" style="padding-bottom: 20px">
            <div class="card-header color-dark fw-500">
                <h4 class="text-capitalize">{{ __('Edit Project')}} ({{($result->project?->name)}})</h4>
            </div>
                                ...
                                <div class="tab-content" id="tabcontent2">
                                        <div class="col-12 col-lg-4 col-md-3">
                                                ...
                                                <input type="text" id="status_name"
                                                       name="status_name"
                                                       class="form-control"
                                                       value="{{ old('status_name', $result->status_name ?? '') }}">
                                            </div>
                                        <div class="col-12 col-lg-4 col-md-3">
                                            <br>
                                            ...
                                            <select required style="font-size: 20px" class="js-example-basic-multiple"
                                                ...
                                                @foreach(\App\Models\Project::where('is_project_case',1)->get() as $project)
                                                    <option value="{{$project->id}}" {{$project->id == $result->project_id ? 'selected':''}}>{{ $project->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12 col-lg-4 col-md-3">
                                        ...
                                        @foreach($languages as $language)
                                                ...
                                                @selected($language->short_name == $result->locale) 
                                    ...
                                    <div class="col-6 mt-20">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" role="switch" value="1"
                                                   name="is_active"
                                                   id="flexSwitchCheckChecked" {{$result->active ==1 ? "checked":''}}>
                                    ...
                                    <button type="button" id="getData"
                                            class="btn btn-secondary">{{__('Edit Form')}}</button>
                                    <br><br>
    ...
    <script>
        jQuery(function ($) {
            ...
            fbEditor.style.display = "none"
            var formBuilder = $(fbEditor).formBuilder(options);
            const old_data = '{{$result->form_data}}'.replace(/&quot;/g, '"');
            document.getElementById('getData').addEventListener('click', function () {
                fbEditor.style.display = "block";
                document.getElementById('getData').style.display = "none"
                formBuilder.actions.setData(old_data);
            })
            ...
            document.getElementById('save-all').addEventListener('click', function () {
                ...
                $.ajax({
                    method: 'PATCH',
                    url: '{{ route("forms.update",$result->id) }}',
                    dataType: 'json',
                    data: {
                        _token: csrfToken,
                        form_data: form_data,
