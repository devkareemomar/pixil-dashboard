...
@section('description', __('Manage tags'))
    ...
    <div class="container-fluid" style="padding-bottom: 24px">
        ...
        <div class="card mb-80">
            <div class="card-header color-dark fw-500">
                <h4 class="text-capitalize">{{ __('Tags') }} ({{ $results->links()->paginator->total() }})</h4>
                <div class="m-1 mt-3">
                    <a class="btn btn-secondary mb-3" style="display: inline-block"
                       href="{{route('tags.export',request()->getQueryString())}}"> {{__('Export Data')}}</a>

                    <a href="{{ route('tags.create') }}" style="display: inline-block"
                       class="btn btn-primary mb-3">{{ __('Create tag') }}</a>
                </div>
            </div>
            <div class="card-body">
                <x-filter :filter_attributes="['name' => 'text', 'slug' => 'text']"/>
                ...
                <form action="{{route('tags.deleteSelectRow')}}" enctype="multipart/form-data" method="post">
                    @csrf
                    @method('DELETE')
                        ...
                        <table class="table mb-20 table-borderless">
                            ...
                            @foreach ($results as $key=>$result)
                                <tr>
                                    ...
                                    <td>{{ $result->slug }}</td>
                                    <td>
                                        <x-action-buttons route="tags" :model="$result" :show="false"/>
                                    </td>
                                </tr>
                            @endforeach
                            @if(count($results)<=0)
